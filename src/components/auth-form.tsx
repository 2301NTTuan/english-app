...
export function AuthForm({ mode }: { mode: "login" | "register" }) {
  ...
  return <div className="grid min-h-screen place-items-center bg-[#f3f7f5] p-4"><div className="card w-full max-w-md p-6 sm:p-8">
    ...
    <form className="mt-6 space-y-4" onSubmit={submit}>
      {registering && <Field label="Name" name="name" type="text" autoComplete="name"/>}<Field label="Email" name="email" type="email" autoComplete="email"/><Field label="Password" name="password" type="password" autoComplete={registering ? "new-password" : "current-password"}/>{!registering && <div className="text-right"><Link className="text-xs font-bold text-[#17795b]" href="/forgot-password">Forgot password?</Link></div>}{registering && <><Field label="Confirm password" name="confirmation" type="password" autoComplete="new-password"/><p className="muted text-xs">Use at least 12 characters with uppercase, lowercase, and a number.</p></>}
