...
export function AuthForm({ mode }: { mode: "login" | "register" }) {
  ...
  return <main className="grid min-h-screen lg:grid-cols-[1.05fr_.95fr]">
    ...
    <form className="mt-6 space-y-4" onSubmit={submit}>
      {registering && <Field label="Name" name="name" type="text" autoComplete="name"/>}<Field label="Email" name="email" type="email" autoComplete="email"/><Field label="Password" name="password" type="password" autoComplete={registering ? "new-password" : "current-password"}/>{!registering && <div className="text-right"><Link className="text-xs font-bold text-[var(--brand)] hover:underline" href="/forgot-password">Forgot password?</Link></div>}{registering && <><Field label="Confirm password" name="confirmation" type="password" autoComplete="new-password"/><p className="muted text-xs">Use at least 12 characters with uppercase, lowercase, and a number.</p></>}
...
function Field({ label, ...props }: { label: string; name: string; type: string; autoComplete: string }) { return <label className="block"><span className="mb-1.5 block text-sm font-bold">{label}</span><input {...props} required className="field h-12"/></label>; }
