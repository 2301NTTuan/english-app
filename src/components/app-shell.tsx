  ...
  return <div className="min-h-screen lg:grid lg:grid-cols-[238px_1fr]">
    ...
    <aside aria-label="Application navigation" className={`${open ? "fixed inset-0 z-50 flex" : "hidden"} bg-black/25 lg:sticky lg:top-0 lg:flex lg:h-screen lg:bg-transparent`} onClick={() => setOpen(false)}>
      <div className="flex h-full w-[265px] flex-col overflow-y-auto border-r border-[#dce6e1] bg-white p-4" onClick={(event) => event.stopPropagation()}>
        ...
        <nav className="space-y-1">{navigation.map(([label, href, Icon]) => { const active = href === "/" ? pathname === "/" : pathname.startsWith(href); return <Link key={href} href={href} prefetch={href === "/grammar" ? false : undefined} aria-current={active ? "page" : undefined} onClick={() => setOpen(false)} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold ${active ? "bg-[#e8f6f0] text-[#11664b]" : "text-[#586760] hover:bg-[#f3f6f4]"}`}><Icon size={18}/>{label}</Link>; })}</nav>
    ...
    <div className="min-w-0">
      ...
      <nav aria-label="Primary mobile navigation" className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-5 border-t border-[#dce6e1] bg-white px-1 py-1.5 lg:hidden">{navigation.slice(0, 5).map(([label, href, Icon]) => { const active = href === "/" ? pathname === "/" : pathname.startsWith(href); return <Link key={href} href={href} prefetch={href === "/grammar" ? false : undefined} aria-current={active ? "page" : undefined} className={`flex min-h-12 flex-col items-center justify-center gap-1 rounded-lg py-1 text-[10px] font-bold ${active ? "text-[#17795b]" : "text-[#718078]"}`}><Icon size={19}/>{label}</Link>; })}</nav>
    </div>
  </div>;
}
