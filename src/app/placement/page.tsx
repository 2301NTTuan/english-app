...
export default function PlacementPage() {
  ...
  const [startedAt, setStartedAt] = useState(() => new Date().toISOString()); const [idempotencyKey, setIdempotencyKey] = useState(() => crypto.randomUUID());
  ...
  const choose = async (answer: string) => {
    ...
    if (next.length >= PLACEMENT_LENGTH) {
      ...
      setState(nextState); setPending(payload);
