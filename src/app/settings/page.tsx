...
export default function SettingsPage() {
  ...
  return <>
    ...
    <div aria-live="polite">{notice && <div className={`feedback mb-4 text-sm font-bold ${notice.kind === "success" ? "feedback-success" : "feedback-error"}`}>{notice.message}</div>}</div>
    <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_360px]">
      <section className="card p-5 sm:p-6">
        <h2 className="text-lg font-[820] text-[var(--ink-strong)]">Learning preferences</h2>
        <div className="mt-6 space-y-7">
          <label className="block"><div className="mb-2 flex justify-between"><b className="text-sm">Current CEFR level</b><span className="badge">{settings.currentLevel}</span></div><select value={settings.currentLevel} onChange={(event) => setDraft({ ...settings, currentLevel: event.target.value as CEFRLevel })} className="select-field">{["A1", "A2", "B1", "B2", "C1", "C2"].map((level) => <option key={level}>{level}</option>)}</select></label>
          ...
          <label className="flex items-center justify-between gap-4 rounded-xl border border-[var(--line)] bg-[var(--surface-muted)] p-4"><span><b className="block text-sm">Show Vietnamese support</b><span className="muted text-xs">Meanings remain English-first.</span></span><input type="checkbox" checked={settings.showVietnamese} onChange={(event) => setDraft({ ...settings, showVietnamese: event.target.checked })} className="size-5 accent-[var(--brand)]"/></label>
          <label className="block"><div className="mb-2 text-sm font-bold">Interface language</div><select value={settings.interfaceLanguage} onChange={(event) => setDraft({ ...settings, interfaceLanguage: event.target.value as "en" | "vi" })} className="select-field"><option value="en">English</option><option value="vi">Vietnamese</option></select></label>
        </div>
      ...
      <div className="space-y-5">
        <section className="card p-5"><div className="eyebrow">Appearance</div><h2 className="mt-2 font-[820] text-[var(--ink-strong)]">Color theme</h2><p className="muted mb-4 mt-1 text-sm">Choose the comfortable reading mode for this device.</p><ThemeToggle/></section>
        <aside className="card p-5"><div className="eyebrow">How adaptation works</div><h2 className="mt-2 text-lg font-[820] text-[var(--ink-strong)]">Reviews protect memory</h2><ol className="muted mt-4 space-y-3 text-sm"><li><b className="text-[var(--ink-strong)]">1.</b> Overdue and due reviews always stay in the plan.</li><li><b className="text-[var(--ink-strong)]">2.</b> Weak dimensions and recurring mistakes come next.</li><li><b className="text-[var(--ink-strong)]">3.</b> New words use remaining target capacity.</li><li><b className="text-[var(--ink-strong)]">4.</b> A full backlog reduces new words to zero.</li></ol></aside>
        ...
        <section className="card border-[color-mix(in_srgb,var(--danger)_35%,var(--line))] p-5"><h2 className="font-extrabold text-[var(--danger)]">Delete account</h2><p className="muted mt-1 text-sm">This permanently deletes your account and learning data. Export a backup first if needed.</p><label className="mt-4 block text-xs font-bold">Type DELETE<input value={deleteConfirmation} onChange={(event) => setDeleteConfirmation(event.target.value)} className="field mt-1 min-h-10 border-[color-mix(in_srgb,var(--danger)_35%,var(--line))]"/></label><label className="mt-3 block text-xs font-bold">Current password<input type="password" autoComplete="current-password" value={deletePassword} onChange={(event) => setDeletePassword(event.target.value)} className="field mt-1 min-h-10 border-[color-mix(in_srgb,var(--danger)_35%,var(--line))]"/></label><button className="mt-4 flex min-h-10 items-center gap-2 rounded-xl bg-[var(--danger)] px-4 py-2 text-sm font-bold text-white hover:opacity-90" onClick={() => void deleteAccount()}><Trash2 size={15}/>Delete account</button></section>
      </div>
    </div>
  </>;
}
...
function Range({ label, value, min, max, suffix, onChange }: { label: string; value: number; min: number; max: number; suffix: string; onChange: (value: number) => void }) {
  return <label className="block"><div className="mb-2 flex justify-between"><b className="text-sm">{label}</b><span className="text-sm font-extrabold text-[var(--brand)]">{value} {suffix}</span></div><input type="range" min={min} max={max} value={value} onChange={(event) => onChange(Number(event.target.value))} className="w-full accent-[var(--brand)]"/><div className="muted mt-1 flex justify-between text-[10px]"><span>{min}</span><span>{max}</span></div></label>;
}
