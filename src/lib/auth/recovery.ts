...
export const hashOneTimeToken = (token: string) => createHash("sha256").update(token).digest("hex");
export const verificationExpiresAt = (now = new Date(), ttlMs = VERIFY_TTL_MS) => new Date(now.getTime() + ttlMs);
export type EmailVerificationResult = "verified" | "already-verified" | "expired" | "invalid";
...
export async function issuePasswordReset(email: string, now = new Date(), ttlMs = RESET_TTL_MS): Promise<string | null> {
  ...
  await getDb().transaction(async (tx) => {
    ...
    await tx.insert(passwordResetTokens).values({ tokenHash: hashOneTimeToken(token), userId: user.id, expiresAt: new Date(now.getTime() + ttlMs) });
...
export async function consumePasswordReset(token: string, password: string, now = new Date()): Promise<boolean> {
  ...
  return getDb().transaction(async (tx) => {
    const [claimed] = await tx.update(passwordResetTokens).set({ usedAt: now }).where(and(
      eq(passwordResetTokens.tokenHash, hashOneTimeToken(token)), isNull(passwordResetTokens.usedAt), gt(passwordResetTokens.expiresAt, now),
...
export async function issueEmailVerification(userId: string, now = new Date(), ttlMs = VERIFY_TTL_MS): Promise<string> {
  ...
  await getDb().transaction(async (tx) => {
    ...
    await tx.insert(emailVerificationTokens).values({ tokenHash: hashOneTimeToken(token), userId, expiresAt: verificationExpiresAt(now, ttlMs) });
    await tx.insert(auditLogs).values({ userId, action: "account.email_verification_requested", entityType: "user", entityId: userId });
  });
  return token;
}

export async function consumeEmailVerification(token: string, now = new Date()): Promise<EmailVerificationResult> {
  return getDb().transaction(async (tx) => {
    ...
    }).from(emailVerificationTokens).innerJoin(users, eq(emailVerificationTokens.userId, users.id))
      .where(eq(emailVerificationTokens.tokenHash, tokenHash)).limit(1);
    if (!record) return "invalid";
    if (record.usedAt) return record.emailVerifiedAt ? "already-verified" : "invalid";
    if (record.expiresAt <= now) return "expired";
    const [claimed] = await tx.update(emailVerificationTokens).set({ usedAt: now }).where(and(
      eq(emailVerificationTokens.tokenHash, tokenHash), isNull(emailVerificationTokens.usedAt), gt(emailVerificationTokens.expiresAt, now),
    )).returning({ userId: emailVerificationTokens.userId });
    if (!claimed) {
      const [user] = await tx.select({ emailVerifiedAt: users.emailVerifiedAt }).from(users).where(eq(users.id, record.userId)).limit(1);
      return user?.emailVerifiedAt ? "already-verified" : "invalid";
    }
